...
class DeepLab_v3_plus(nn.Module):
    ...
    def __init__(self, n_classes, n_blocks, atrous_rates, multi_grids, output_stride):
        ...
        self.layer2 = _ResStage(n_blocks=n_blocks[0], in_ch=ch[0], out_ch=ch[2], stride=s[0], dilation=d[0])
        self.layer3 = _ResStage(n_blocks=n_blocks[1], in_ch=ch[2], out_ch=ch[3], stride=s[1], dilation=d[1])
        self.layer4 = _ResStage(n_blocks=n_blocks[2], in_ch=ch[3], out_ch=ch[4], stride=s[2], dilation=d[2])
        self.layer5 = _ResStage(n_blocks=n_blocks[3], in_ch=ch[4], out_ch=ch[5], stride=s[3], dilation=d[3], multi_grids=multi_grids)
        self.aspp = _ASPP(in_ch=ch[5], out_ch=256, rates=atrous_rates)
        concat_ch = 256 * (len(atrous_rates) + 2)
        self.fc1 = _ConvBnReLU(in_ch=concat_ch, out_ch=256, kernel_size=1, stride=1, padding=0, dilation=1)

        # Decoder
        self.reduce_channel = _ConvBnReLU(in_ch=256, out_ch=48, kernel_size=1, stride=1, padding=0, dilation=1)
        self.fc2 = nn.Sequential(OrderedDict([
            ("conv1", _ConvBnReLU(in_ch=304, out_ch=256, kernel_size=3, stride=1, padding=1, dilation=1)),
            ("conv2", _ConvBnReLU(in_ch=256, out_ch=256, kernel_size=3, stride=1, padding=1, dilation=1)),
            ("conv3", nn.Conv2d(in_channels=256, out_channels=n_classes, kernel_size=1)),
        ]))

    def forward(self, x):
        h = self.layer1(x)
        h = self.layer2(h)
        h_ = self.reduce_channel(h)
        h = self.layer3(h)
        h = self.layer4(h)
        h = self.layer5(h)
        h = self.aspp(h)
